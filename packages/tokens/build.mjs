...
const PRESETS = ['hi-vis', 'sea-glass', 'bauhaus', 'earthy', 'tropical', 'sunset-berry', 'bodega', 'indigo-pop'];
...
async function buildVariant(preset, mode) {
  const sd = new StyleDictionary({
    ...
    platforms: {
      ...
      ts: {
        transforms: ['attribute/cti', 'name/snake', 'valence/strip-unit', 'valence/native-type-scale'],
...
const combined = [
  ...
  ...PRESETS.flatMap((preset) => MODES.map((mode) => readFileSync(`dist/css/${preset}-${mode}.css`, 'utf8'))),
].join('\n');
writeFileSync('dist/css/tokens.css', `/* Generated by build.mjs — do not edit by hand. */\n\n${combined}`);

console.log(`Tokens built: dist/css/tokens.css, src/generated/{${PRESETS.map((p) => `${p}-{light,dark}`).join(',')}}.ts`);
